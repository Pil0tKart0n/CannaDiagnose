...
async function sendFeedbackToServer(
  ...
  if (rating === 'negative' && images && images.length > 0) {
    try {
      const base64Images = await Promise.all(images.slice(0, 5).map(imageToDataUri));
...
export default function ResultsScreen() {
  ...
  const displayResult: DiagnosisResult | null = refinedResult
    ...
    : params.historyResult
      ? (() => {
          try {
            return validateDiagnosisResult(JSON.parse(params.historyResult));
          } catch {
            return result;
          }
        })()
  ...
  const historyImagesParsed: string[] = params.historyImages
    ? (() => {
        try {
          return JSON.parse(params.historyImages);
        } catch {
          return [];
        }
      })()
  ...
  const handleRefine = async () => {
    ...
    if (!hasData) {
      const msg = isLivingSoil || isOrganic ? t('results.missingDataOrganic') : t('results.missingDataMineral');
    ...
    if (phInput) {
      ...
      if (isNaN(ph) || ph < 0 || ph > 14) {
        ...
        if (Platform.OS === 'web' && typeof window !== 'undefined') {
          ...
          Alert.alert(t('results.invalidValue'), msg);
        }
        return;
      }
    }
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      >
        ...
        >
          ...
          {displayResult.contributingFactors?.length > 0 && <FactorsList factors={displayResult.contributingFactors} />}
          ...
          {!isFromHistory && !(refined && colorCorrected) && (
            <View
              onLayout={(e) => {
                refineYRef.current = e.nativeEvent.layout.y;
              }}
            >
              <RefineCard
          ...
          {(() => {
            ...
            const match = libraryEntries.find((e) => {
              const _n = e.name.toLowerCase();
              if (diag.includes('stickstoff') || diag.includes('(n)') || diag.includes('n-mangel'))
                return e.id === 'n-stickstoff';
              ...
              if (diag.includes('kalzium') || diag.includes('calcium') || diag.includes('(ca)'))
                return e.id === 'n-kalzium';
              ...
              <View style={styles.feedbackButtons}>
                ...
                >
                  <Ionicons
                    ...
                    size={18}
                    color={feedback === 'positive' ? colors.accent : colors.textMuted}
                  />
                </TouchableOpacity>
                <TouchableOpacity
                ...
                >
                  <Ionicons
                    ...
                    size={18}
                    color={feedback === 'negative' ? colors.error : colors.textMuted}
                  />
                </TouchableOpacity>
              </View>
            </View>
